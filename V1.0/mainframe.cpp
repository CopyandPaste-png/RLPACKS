	...
	Error{ std::unique_ptr<logging>(new logging("error.txt"))}
{
	...
	//Create box
	mbox = std::unique_ptr<wxListBox>(new wxListBox(this, 23, wxPoint(10, 10), wxSize(360, 500)));
	mbox->Append("Results appear here");
	//Create text area for user input
	
	//searching
	searchingTerm = std::unique_ptr<wxTextCtrl>(new wxTextCtrl(this, 10, wxString("Enter search term here"), wxPoint(400, 30), wxSize(195, 30)));
	searchingTag = std::unique_ptr<wxTextCtrl>(new wxTextCtrl(this, 11, wxString("Enter tag term here"), wxPoint(400, 60), wxSize(195, 30)));

	//inserting
	insertingName = std::unique_ptr<wxTextCtrl>(new wxTextCtrl(this, 12, wxString("Enter name of pack"), wxPoint(400, 200), wxSize(195, 30)));
	insertingCode = std::unique_ptr<wxTextCtrl>(new wxTextCtrl(this, 13, wxString("Enter code of pack"), wxPoint(400, 230), wxSize(195, 30)));
	insertingTag = std::unique_ptr<wxTextCtrl>(new wxTextCtrl(this, 14, wxString("Enter tags of pack(seperated by ',')"), wxPoint(400, 260), wxSize(195, 30)));

	//status
	//wxTE_READONLY
	status = std::unique_ptr<wxTextCtrl>(new wxTextCtrl(this, 15, wxString("Status of events here"), wxPoint(400, 400), wxSize(250, 30), wxTE_READONLY));
	...
	//Create buttons
	search = std::unique_ptr<wxButton>(new wxButton(this, 20, "search", wxPoint(400, 120), wxSize(100, 30)));
	clear = std::unique_ptr<wxButton>(new wxButton(this, 21, "clear", wxPoint(400, 150), wxSize(100, 30)));
	insert = std::unique_ptr<wxButton>(new wxButton(this, 22, "insert", wxPoint(400, 320), wxSize(100, 30))); 
	...
	Bind(wxEVT_CHAR_HOOK, &mainframe::OnEnterHit, this);
...
void mainframe::OnEnterHit(wxKeyEvent& evt) {
	if ((int)evt.GetKeyCode()==13){ //enter pressed
		searchPacks();
	}
