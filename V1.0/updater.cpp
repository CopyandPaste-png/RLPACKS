...
{}
void update::updateApp(std::string new_version) {
	...
	case S_OK:
		...
		files::write_to_file_Version(new_version);
		startNewProcess();
		break;
	case E_OUTOFMEMORY:
...
bool update::versionsEqual() {
	//Returns opposite of its value
	return !(files::readCurrentVersionFile() == web::ghVersion());
}